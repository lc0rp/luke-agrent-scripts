...
```bash
# Pick a nickname. Format <ANIMAL>-<PLACE> or <COLOR>-<NOUN> or your own. Stick with it.
bd ready              # Find available work (no blockers)
bd list --status=open # All open issues
bd show <id>          # View issue details with dependencies
bd update <id> --claim --assignee <your-nickname>  # Claim work. MUST run before starting work. Must include --claim and --assignee to prevent race conditions.
...
2. **Claim**: Use `bd update <id> --claim --assignee <your-nickname>` to claim the work
...
6. **Git/bd atomic commit & push**: - THIS IS MANDATORY AFTER EVERY BEAD CLOSE
    ...
    ```
7. **Clean up** - Prune remote branches (do not clear stashes in multi-agent repos)
