...
by calling `codex exec` as a subprocess (same auth pattern as run_eval.py —
uses the session's Codex auth, no separate API KEY needed).
"""
...
def _call_codex(prompt: str, model: str | None, timeout: int = 300) -> str:
    """Run `codex exec` with the prompt on stdin and return the text response.
...
Please respond with only the new description text in <new_description> tags, nothing else."""
    ...
    # SDK path did this as a true multi-turn; `codex exec` is one-shot, so we
