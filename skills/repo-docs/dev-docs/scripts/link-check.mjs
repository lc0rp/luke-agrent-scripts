...
function convertWikilinks(content, filePath) {
  ...
  mutated = mutated.replace(pattern, (match, target, alias) => {
    ...
    unresolved.push({ file: filePath, target, reason: resolved.status, candidates: resolved.candidates || [] });
    return match;
  });
...
if (!fix && needsConversion.length) {
  errors.push(`Found ${needsConversion.length} wikilinks that need conversion (run pnpm run lint:links:fix).`);
}
if (unresolvedLinks.length) {
  const lines = unresolvedLinks.slice(0, 10).map((u) => `- ${u.file}: [[${u.target}]] (${u.reason})`);
  errors.push(`Unresolved/ambiguous wikilinks:\n${lines.join('\n')}${unresolvedLinks.length > 10 ? '\n…' : ''}`);
}
if (deadLinks.length) {
  const lines = deadLinks.slice(0, 10).map((d) => `- ${d.file}: ${d.link}`);
  errors.push(`Dead links:\n${lines.join('\n')}${deadLinks.length > 10 ? '\n…' : ''}`);
}
